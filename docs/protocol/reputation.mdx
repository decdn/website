...
A single interaction moves a peer's score by at most **±0.05**, so one bad exchange cannot destroy a good peer's standing. With α = 0.1 the EWMA itself keeps most deltas well under that; the clamp only binds when the gap between the peer's score and the interaction exceeds 0.5.
