...
- **Corrupted delivery.** Absorbed at the wire: the client verifies each chunk group as it arrives and signs no voucher for a corrupt window, so the node ships garbage unpaid. There is no on-chain path.
