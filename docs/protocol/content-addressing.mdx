...
Because every chunk group carries its own proof, a resumed or ranged fetch verifies **independently from any byte offset** once it aligns to the next unverified chunk group — no need to re-read from the start. Payment meters transmitted bytes, and the proof bytes are transmitted, so they are metered too: the verification overhead is roughly **0.4%** of blob size, not free metadata.
