...
3. **Close** — either party submits the latest voucher on-chain. A dispute window lets the counterparty submit a later-nonce voucher if the close is stale. When both parties are online and agree, a single co-signed transaction settles immediately with no dispute window (the cooperative fast path), falling back to the standard close if either side declines.
