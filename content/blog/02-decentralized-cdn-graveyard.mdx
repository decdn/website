...
decdn is the protocol composed to avoid all six. We pay in USDC; we settle on a public L2; every byte is paid including node-to-node cache pulls; the architecture is delivery-first; the on-chain blacklist with regional governance is in the protocol from day one; and we did not invent any of the underlying primitives.

The graveyard is informative not because each attempt was foolish, but because each one was held back by something concrete. Those things are now resolved. That's the whole pitch.
