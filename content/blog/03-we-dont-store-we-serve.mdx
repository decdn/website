...
- The network surface is small. We don't carry an S3 protocol, a B2 protocol, a NFS shim. There's one small origin adapter — fetch the bytes for a given hash, and report what kind of backend it is — and operators implement it for whichever backend they have.
