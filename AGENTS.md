...
- **`metadataBase` is live.** `lib/links.ts` `site` is the real origin and `INDEXABLE` is `true`. Anything anchored on this origin — OG, JSON-LD, canonical (via `metadataBase`), and `app/sitemap.ts` / `app/robots.ts` (via `SITE_URL`) — ships to production. Adding a new page = append an entry to `app/sitemap.ts`; don't add config elsewhere. Flip `INDEXABLE` in `lib/links.ts` to take the site out of search; only the meta tag flips, `robots.txt` always allows crawling (Disallow + noindex is an anti-pattern that strands URLs in the index).
