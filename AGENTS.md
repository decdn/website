...
- **Section vertical rhythm.** `Frame` owns inter-section spacing via `py-[var(--frame-pad-y)]`. `Footer` owns the gap _above itself_ via `pt-[var(--frame-pad-y)]`; `globals.css` zeroes the `padding-bottom` of the section directly before the footer (`main:has(+ footer) > section:last-child`) so the unit isn't doubled (the #123/#125 empty band). Don't add `pt-*` to `Footer` on top of this, and don't reintroduce the footer's reliance on the preceding block's bottom padding.
